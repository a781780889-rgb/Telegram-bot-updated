...
bot.action(/^code_type_(month|year|open)$/, async (ctx) => userCodes.handleCreateType(ctx, ctx.match[1]));
bot.action('codes_batch', userCodes.handleBatchStart);
bot.action(/^batch_type_(month|year|open)$/, async (ctx) => userCodes.handleBatchType(ctx, ctx.match[1]));
